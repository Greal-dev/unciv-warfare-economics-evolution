...
class AlertPopup(
    ...
    private fun addCityConquered() {
        ...
        if (conqueringCiv.isOneCityChallenger()) {
            ...
            // TW: Vassalize option - capital must be taken (now or previously) + defeated civ has <= 3 cities
            val defeatedCiv = city.civ
            val capitalTaken = city.isOriginalCapital || defeatedCiv.cities.none { it.isOriginalCapital }
            val canVassalize = capitalTaken && defeatedCiv.isMajorCiv() && !conqueringCiv.isVassal()
                    && defeatedCiv.cities.size <= 3 && !defeatedCiv.isVassal()
            if (canVassalize) {
                addVassalizeOption(city, conqueringCiv)
                addSeparator()
            } else if (defeatedCiv.isMajorCiv() && !conqueringCiv.isVassal()) {
                // Debug info: show why vassalization is not available
                val reasons = mutableListOf<String>()
                if (!capitalTaken) reasons.add("capital not taken")
                if (defeatedCiv.cities.size > 3) reasons.add("${defeatedCiv.cities.size} cities (max 3)")
                if (defeatedCiv.isVassal()) reasons.add("already a vassal")
                if (reasons.isNotEmpty()) {
                    add("Vassalize unavailable: ${reasons.joinToString(", ")}".toLabel(Color.GRAY, 12)).colspan(2).row()
                }
            }
    ...
    private fun addVassalizeOption(city: com.unciv.logic.city.City, conqueringCiv: com.unciv.logic.civilization.Civilization) {
        val button = "Vassalize".toTextButton()
        button.onActivation {
            com.unciv.logic.city.managers.CityConquestFunctions(city).vassalizeCity(conqueringCiv)
            close()
        }
        button.keyShortcuts.add('v')
        bottomTable.add(button).row()
    }

    /** TW: Vassal requests independence — suzerain can accept or refuse */
    private fun addVassalIndependenceRequest(): Boolean {
        val vassal = getCiv(popupAlert.value)
        if (vassal.isDefeated() || !vassal.isVassal()) return false

        addGoodSizedLabel("[${vassal.civName}] requests independence!")
        addSeparator()
        addGoodSizedLabel("Our vassal [${vassal.civName}] demands to be released from our control. If we refuse, they will declare war.")
        addSeparator()

        // Accept — peaceful independence
        val acceptButton = "Grant independence".toTextButton()
        acceptButton.onActivation {
            vassal.releaseFromVassalage()
            vassal.addNotification("[${viewingCiv.civName}] has granted us independence!", NotificationCategory.Diplomacy, NotificationIcon.Diplomacy)
            viewingCiv.addNotification("We have released [${vassal.civName}] from vassalage.", NotificationCategory.Diplomacy, NotificationIcon.Diplomacy)
            close()
        }
        acceptButton.keyShortcuts.add('a')
        bottomTable.add(acceptButton).pad(5f)

        // Refuse — independence war
        val refuseButton = "Refuse".toTextButton()
        refuseButton.onActivation {
            vassal.declareIndependence()
            close()
        }
        refuseButton.keyShortcuts.add('r')
        bottomTable.add(refuseButton).pad(5f)
        bottomTable.row()
