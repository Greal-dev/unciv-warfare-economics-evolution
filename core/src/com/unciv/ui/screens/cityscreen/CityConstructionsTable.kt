...
class CityConstructionsTable(private val cityScreen: CityScreen) {
    ...
    private fun getConstructionButtonDTOs(): ArrayList<ConstructionButtonDTO> {
        ...
        // Add production transfer options (one per other city)
        for (targetCity in city.civ.cities) {
            if (targetCity == city) continue
            val transfer = ProductionTransferConstruction(targetCity.id, targetCity.name)
            if (transfer.shouldBeDisplayed(cityConstructions)) {
                constructionButtonDTOList.add(
                    ConstructionButtonDTO(
                        transfer,
                        transfer.getDisplayName().tr() + " " + transfer.getProductionTooltip(city).trim()
                    )
                )
            }
        }
    ...
    private fun getQueueEntry(constructionQueueIndex: Int, constructionName: String): Table {
        ...
        var text = if (construction is ProductionTransferConstruction) construction.getDisplayName().tr(true)
                else constructionName.tr(true)
        text += if (construction is ProductionTransferConstruction) "\n" + Fonts.infinity
                else if (constructionName in PerpetualConstruction.perpetualConstructionsMap) "\n" + Fonts.infinity
