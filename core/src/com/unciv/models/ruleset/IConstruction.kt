...
class ProductionTransferConstruction(val targetCityId: String, val targetCityName: String) :
    ...
    override fun isBuildable(cityConstructions: CityConstructions): Boolean {
        val sourceCity = cityConstructions.city
        val targetCity = sourceCity.civ.cities.firstOrNull { it.id == targetCityId } ?: return false
        if (targetCity == sourceCity) return false
        if (targetCity.isInResistance()) return false
        return true
    }

    override fun shouldBeDisplayed(cityConstructions: CityConstructions) = isBuildable(cityConstructions)

    @Readonly override fun getProductionTooltip(city: City, withIcon: Boolean): String {
        val efficiency = getEfficiency(city.civ)
        val transferred = (city.cityStats.currentCityStats.production * efficiency).roundToInt()
        return "\r\n$transferred${if (withIcon) Fonts.production else ""}/${Fonts.turn} (${(efficiency * 100).roundToInt()}%)"
    }

    @Readonly fun getDisplayName(): String = "Transfer Production to [$targetCityName]"
    ...
    companion object {
        const val PREFIX = "Transfer Production@"

        @Readonly @JvmStatic fun isTransferConstruction(name: String) = name.startsWith(PREFIX)

        @Readonly @JvmStatic fun parseCityId(name: String): String? {
            if (!isTransferConstruction(name)) return null
            return name.removePrefix(PREFIX)
        }

        @Readonly @JvmStatic fun getEfficiency(civ: Civilization): Float {
            val maxEraNumber = civ.gameInfo.ruleset.eras.values.maxOfOrNull { it.eraNumber } ?: 0
            val civEraNumber = civ.getEra().eraNumber
            val efficiency = 1.0f - (maxEraNumber - civEraNumber) * 0.10f
            return max(0.1f, min(1.0f, efficiency))
        }
    }
}
