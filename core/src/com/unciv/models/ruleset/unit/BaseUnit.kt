...
class BaseUnit : RulesetObject(), INonPerpetualConstruction {
    ...
    fun construct(cityConstructions: CityConstructions, boughtWith: Stat?): MapUnit? {
        ...
        // TW: Workers and settlers cost 2 population each to produce
        if (boughtWith == null) { // only when produced, not purchased
            val city = cityConstructions.city
            val isWorker = hasUnique(UniqueType.BuildImprovements)
            val isSettler = hasUnique(UniqueType.FoundCity)
            if ((isWorker || isSettler) && city.population.population > 2) {
                city.population.addPopulation(-2)
            }
        }
