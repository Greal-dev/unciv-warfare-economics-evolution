...
class CityPopulationManager : IsPartOfGameInfoSerialization {
    ...
    fun getNumTurnsToStarvation(): Int? {
        ...
        // TW: 1 pop lost every 2 turns during famine, population dies at remaining turns × 2
    ...
    fun getNumTurnsToNewPopulation(): Int? {
        ...
        // TW: food accumulates at 2× speed
        val effectiveFoodPerTurn = city.foodForNextTurn().toFloat() * 2f
        val remainingFood = getFoodToNextPopulation() - foodStored
        var turnsToGrowth = ceil(remainingFood / effectiveFoodPerTurn).toInt()
    ...
    fun nextTurn(food: Int) {
        ...
        if (food < 0) {
            // No food stockpile — foodStored stays at 0 during famine
            foodStored = 0
            ...
            // Lose 1 population every 2 turns of famine
            if (population > 1 && city.civ.gameInfo.turns % 2 == 0) {
                addPopulation(-1)
                city.civ.addNotification("[${city.name}] has lost a citizen to famine!",
                    city.location, NotificationCategory.Cities, NotificationIcon.Death)
            }
            return
        }
