...
class CityCombatant(val city: City) : ICombatant {
    ...
    fun getCityStrength(theirCombatant: ICombatant? = null, combatAction: CombatAction = CombatAction.Defend): Int { // Civ fanatics forum, from a modder who went through the original code
        ...
        // Territorial Warfare: small nation city defense bonus (major civs only)
        if (getCivInfo().isMajorCiv()) {
            val cityCount = getCivInfo().cities.size
            val smallNationMultiplier = when (cityCount) {
                1 -> 2.0f
                2 -> 1.5f
                3 -> 1.25f
                else -> 1.0f
            }
            strength *= smallNationMultiplier
        }
