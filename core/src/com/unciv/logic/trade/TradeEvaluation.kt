...
class TradeEvaluation {
    ...
    fun getTradeAcceptability(trade: Trade, evaluator: Civilization, tradePartner: Civilization, includeDiplomaticGifts:Boolean = false): Int {
        // Territorial Warfare: civs with no cities always accept peace
        if (evaluator.cities.isEmpty() && trade.isPeaceTreaty()) return 1000
        ...
        when (offer.type) {
            ...
            TradeOfferType.Treaty -> {
                return when (offer.name) {
                    ...
                    // TW: Receiving a vassal is very valuable (25% gold+science tribute)
                    Constants.vassalage -> {
                        val vassalGold = tradePartner.stats.statsForNextTurn.gold.coerceAtLeast(0f)
                        val vassalScience = tradePartner.stats.statsForNextTurn.science.coerceAtLeast(0f)
                        ((vassalGold + vassalScience) * 0.25f * 20).toInt().coerceAtLeast(500)
                    }
                    else -> 1000
                }
            }
        ...
        when (offer.type) {
            ...
            TradeOfferType.Treaty -> {
                return when (offer.name) {
                    ...
                    Constants.researchAgreement -> -offer.amount
                    // TW: Becoming a vassal is extremely costly — only accept when losing badly
                    Constants.vassalage -> {
                        val myGold = civInfo.stats.statsForNextTurn.gold.coerceAtLeast(0f)
                        val myScience = civInfo.stats.statsForNextTurn.science.coerceAtLeast(0f)
                        ((myGold + myScience) * 0.25f * 30).toInt().coerceAtLeast(2000)
                    }
                    else -> 1000
                }
            }
