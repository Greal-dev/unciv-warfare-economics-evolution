...
object TileDescription {
    ...
    fun toMarkup(tile: Tile, viewingCiv: Civilization?, hideUnits: Boolean = false, spyCity: City? = null): ArrayList<FormattedLine> {        val lineList = ArrayList<FormattedLine>()
        ...
        // Territorial Warfare: show cultural composition
        if (tile.cultureMap.isNotEmpty()) {
            lineList += FormattedLine("")
            lineList += FormattedLine("--- Population ---", color = "#AAA")
            // Sort by percentage descending
            val sorted = tile.cultureMap.entries.sortedByDescending { it.value }
            for ((civName, share) in sorted) {
                if (share < 0.01f) continue
                val pct = "%.1f".format(share * 100)
                val color = when {
                    civName == tile.getOwner()?.civName -> "#8F8" // green for owner
                    civName == "Barbarians" -> "#F88"              // red for barbarians
                    else -> "#FF8"                                  // yellow for foreign
                }
                lineList += FormattedLine("$civName: $pct%", color = color)
            }
            if (tile.rebellionTurns > 0) {
                lineList += FormattedLine("REBELLION (${tile.rebellionTurns} turns)", color = "#F44")
            }
