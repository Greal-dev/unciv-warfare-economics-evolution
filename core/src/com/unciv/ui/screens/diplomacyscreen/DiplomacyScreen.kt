...
class DiplomacyScreen(
    ...
    internal fun updateLeftSideTable(selectCiv: Civilization?) {
        ...
        for (civ in viewingCiv.diplomacyFunctions.getKnownCivsSorted()) {
            ...
            // TW: Vassal/Suzerain tag under civ name
            val vassalTag = when {
                civ.isVassal() && civ.vassalOf == viewingCiv.civName -> "(Vassal)".toLabel(Color.CYAN)
                viewingCiv.isVassal() && viewingCiv.vassalOf == civ.civName -> "(Suzerain)".toLabel(Color.PURPLE)
                else -> null
            }
            ...
            val civButton = Table().apply {
                ...
                if (vassalTag != null) add(vassalTag).row()
                onClick {
                    updateRightSide(civ)
                    highlightCiv(this)
                }
    ...
    internal fun getRelationshipTable(otherCivDiplomacyManager: DiplomacyManager): Table {
        ...
        // TW: Vassal/Suzerain status indicator
        val otherCiv = otherCivDiplomacyManager.civInfo
        if (otherCiv.isVassal() && otherCiv.vassalOf == viewingCiv.civName) {
            relationshipTable.add("{Vassal status}: ".toLabel())
            relationshipTable.add("Our Vassal".toLabel(Color.CYAN)).row()
        } else if (viewingCiv.isVassal() && viewingCiv.vassalOf == otherCiv.civName) {
            relationshipTable.add("{Vassal status}: ".toLabel())
            relationshipTable.add("Our Suzerain".toLabel(Color.PURPLE)).row()
        }
