...
class ConstructionInfoTable(val cityScreen: CityScreen) : Table() {
    ...
    private fun updateSelectedConstructionTable(construction: IConstruction) {
        ...
        selectedConstructionTable.run {
            ...
            var buildingText = if (construction is ProductionTransferConstruction) construction.getDisplayName().tr(hideIcons = true)
                    else construction.name.tr(hideIcons = true)
            val specialConstruction = PerpetualConstruction.perpetualConstructionsMap[construction.name]

            buildingText += when (construction) {
                is ProductionTransferConstruction -> construction.getProductionTooltip(city)
                else -> specialConstruction?.getProductionTooltip(city)
            ...
            val description = when (construction) {
                ...
                is ProductionTransferConstruction -> {
                    val efficiency = ProductionTransferConstruction.getEfficiency(city.civ)
                    "Transfer production to another city at ${(efficiency * 100).toInt()}% efficiency (based on current era).".tr()
                }
