...
class TradeLogic(val ourCivilization: Civilization, val otherCivilization: Civilization) {
    ...
    private fun getAvailableOffers(civInfo: Civilization, otherCiv: Civilization): TradeOffersList {
        ...
        if (civInfo.isAtWarWith(otherCiv)) {
            ...
            // TW: Offer to become vassal during peace negotiations (only major civs, not already a vassal)
            if (civInfo.isMajorCiv() && otherCiv.isMajorCiv() && !civInfo.isVassal() && !otherCiv.isVassal())
                offers.add(TradeOffer(Constants.vassalage, TradeOfferType.Treaty, speed = civInfo.gameInfo.speed))
        }
        ...
        fun transferTrade(from: Civilization, to: Civilization, offer: TradeOffer) {
            when (offer.type) {
                ...
                TradeOfferType.Treaty -> {
                    ...
                    // TW: Vassalage — the offering side becomes vassal of the receiving side
                    if (offer.name == Constants.vassalage) {
                        from.vassalOf = to.civName
                        from.vassalTurnEstablished = from.gameInfo.turns
                        from.addNotification("[${to.civName}] is now our suzerain.", com.unciv.logic.civilization.NotificationCategory.Diplomacy, com.unciv.logic.civilization.NotificationIcon.Diplomacy)
                        to.addNotification("[${from.civName}] has accepted vassalage!", com.unciv.logic.civilization.NotificationCategory.Diplomacy, com.unciv.logic.civilization.NotificationIcon.Diplomacy)
                    }
                }
