...
class City : IsPartOfGameInfoSerialization, INamed {
    ...
    /** TW: Smoothed production — converges toward computed potential at 5%/turn of gap.
    ...
    /** TW: previous turn's total production, for capping growth at +5%/turn */
    var previousTurnProduction = 0f
    ...
    fun clone(): City {
        ...
        toReturn.previousTurnProduction = previousTurnProduction
    ...
    @Readonly
    // Territorial Warfare: enemy original capitals can be razed
    fun canBeDestroyed(justCaptured: Boolean = false): Boolean {
        ...
        // Can't raze your own current capital (need to move it first), but captured capitals are fair game
        if (isCapital() && !justCaptured) return false
