...
class TileStatFunctions(val tile: Tile) {
    ...
    fun getTileStatsBreakdown(city: City?, observingCiv: Civilization?,
        ...
        if (improvement != null) {
            // TW: Improvement maturation — stats scale with age
            // 50% at construction, 100% at ~200 turns, ~150% at ~500 turns (sublinear)
            if (tile.improvementTurnBuilt >= 0 && observingCiv != null) {
                val turns = (tile.tileMap.gameInfo.turns - tile.improvementTurnBuilt).coerceAtLeast(0).toDouble()
                val maturation = (0.5 + 0.5 * kotlin.math.ln(1.0 + turns) / kotlin.math.ln(201.0))
                    .coerceAtMost(2.0).toFloat()
                for (stat in Stat.entries) {
                    improvementStats[stat] = improvementStats[stat] * maturation
                }
            }
            listOfStats.add(improvement.name to improvementStats)
        }
