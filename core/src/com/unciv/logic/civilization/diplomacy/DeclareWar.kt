...
object DeclareWar {
    ...
    private fun onWarDeclared(diplomacyManager: DiplomacyManager, isOffensiveWar: Boolean, warType: WarType) {
        ...
        if (diplomacyManager.civInfo.isMajorCiv()) {
            ...
            callInVassals(diplomacyManager)
        }
    ...
    /** TW: Vassals follow their suzerain into war */
    private fun callInVassals(diplomacyManager: DiplomacyManager) {
        val civAtWarWith = diplomacyManager.otherCiv
        for (vassal in diplomacyManager.civInfo.getVassals()) {
            if (vassal.isAtWarWith(civAtWarWith)) continue
            if (!vassal.knows(civAtWarWith))
                vassal.diplomacyFunctions.makeCivilizationsMeet(civAtWarWith, warOnContact = true)
            vassal.getDiplomacyManager(civAtWarWith)?.declareWar(DeclareWarReason(WarType.JoinWar, diplomacyManager.civInfo))
        }
    }
}
