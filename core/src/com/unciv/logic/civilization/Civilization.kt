...
class Civilization : IsPartOfGameInfoSerialization {
    ...
    fun clone(): Civilization {
        ...
        toReturn.vassalOf = vassalOf
        toReturn.vassalTurnEstablished = vassalTurnEstablished
        return toReturn
    }
     ...
     */
    // Territorial Warfare: major civs never die — their culture persists on tiles
    // and can trigger city revolts to resurrect them
    @Readonly
    fun isDefeated() = when {
        isBarbarian || isSpectator() -> false
        isMajorCiv() -> false
    ...
    @Readonly
    internal fun calculateMilitaryMight(): Int {
    ...
    // TW: Vassalage utility methods
    /** TW: Is this civilization a vassal of another? */
    @Readonly fun isVassal() = vassalOf != null
    /** TW: Get the suzerain civilization object */
    @Readonly fun getSuzerain(): Civilization? = if (vassalOf != null) gameInfo.civilizations.firstOrNull { it.civName == vassalOf } else null
    /** TW: Get all vassal civilizations */
    @Readonly fun getVassals(): List<Civilization> = gameInfo.civilizations.filter { it.vassalOf == civName }

    /** TW: Can this vassal request independence? Requires 20-turn cooldown + military might >= 25% of suzerain's */
    fun canDeclareIndependence(): Boolean {
        if (!isVassal()) return false
        val suzerain = getSuzerain() ?: return true  // suzerain gone = auto-free
        // Initialize vassalTurnEstablished for old saves (field defaults to -1)
        if (vassalTurnEstablished < 0) vassalTurnEstablished = gameInfo.turns
        // 20-turn cooldown after vassalization
        if (gameInfo.turns - vassalTurnEstablished < 20) return false
        val myMight = calculateMilitaryMight()
        val suzerainMight = suzerain.calculateMilitaryMight()
        return myMight >= suzerainMight * 0.25f
    }

    /** TW: Release this civilization from vassalage (peaceful or forced) */
    fun releaseFromVassalage() {
        vassalOf = null
        vassalTurnEstablished = -1
    }

    /** TW: Declare independence war against suzerain (when suzerain refuses) */
    fun declareIndependence() {
        val suzerain = getSuzerain() ?: return
        val diploManager = getDiplomacyManager(suzerain)
        releaseFromVassalage()
        if (diploManager != null) {
            diploManager.declareWar(com.unciv.logic.civilization.diplomacy.DeclareWarReason(com.unciv.logic.civilization.diplomacy.WarType.DirectWar))
        }
        addNotification("We have declared our independence from [${suzerain.civName}]!", NotificationCategory.Diplomacy, NotificationIcon.War)
        suzerain.addNotification("[${civName}] has declared independence!", NotificationCategory.Diplomacy, NotificationIcon.War)
    }
