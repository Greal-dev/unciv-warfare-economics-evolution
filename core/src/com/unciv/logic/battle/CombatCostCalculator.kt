...
object CombatCostCalculator {
    ...
    @Readonly
    fun calculate(combatant: ICombatant, hpLost: Int): Int {
        ...
        val raw = hpLost.toFloat() * effectiveEra * distFactor * cultureFactor * K
        return raw.toInt()
    }

    fun applyCost(combatant: ICombatant, hpLost: Int): Int {
        val cost = calculate(combatant, hpLost)
        if (cost <= 0) return 0
