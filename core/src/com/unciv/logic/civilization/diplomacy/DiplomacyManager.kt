...
class DiplomacyManager() : IsPartOfGameInfoSerialization {
    ...
    fun canDeclareWar() = !civInfo.isDefeated() && !otherCiv.isDefeated()
            ...
            // TW: Vassals cannot declare war independently
            && !civInfo.isVassal()
            // TW: Cannot declare war on a vassal directly
            && !otherCiv.isVassal()
    ...
    fun makePeace() {
        ...
        // TW: Vassals follow their suzerain into peace
        for (vassal in civInfo.getVassals()) {
            if (vassal.isAtWarWith(otherCiv)) {
                vassal.getDiplomacyManager(otherCiv)?.makePeace()
            }
        }
        for (vassal in otherCiv.getVassals()) {
            if (vassal.isAtWarWith(civInfo)) {
