...
class CityFounder {
    fun foundCity(civInfo: Civilization, cityLocation: HexCoord, unit: MapUnit? = null): City {
        ...
        // Territorial Warfare: initialize city center culture to 90% founder + 10% surroundings
        val founderName = civInfo.civName
        tile.cultureMap.clear()
        tile.cultureMap[founderName] = 0.90f
        val surroundingCulture = HashMap<String, Float>()
        var neighborCount = 0
        for (neighbor in tile.neighbors) {
            if (neighbor.cultureMap.isNotEmpty()) {
                for ((civName, share) in neighbor.cultureMap) {
                    surroundingCulture[civName] = (surroundingCulture[civName] ?: 0f) + share
                }
                neighborCount++
            }
        }
        if (neighborCount > 0) {
            for ((civName, share) in surroundingCulture) {
                val contribution = (share / neighborCount) * 0.10f
                if (contribution > 0.001f) {
                    tile.cultureMap[civName] = (tile.cultureMap[civName] ?: 0f) + contribution
                }
            }
        } else {
            tile.cultureMap["Barbarians"] = 0.10f
        }
        // Normalize
        val total = tile.cultureMap.values.sum()
        if (total > 0) {
