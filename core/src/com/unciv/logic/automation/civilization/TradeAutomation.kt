...
object TradeAutomation {
    ...
    fun respondToTradeRequests(civInfo: Civilization, tradeAndChangeState: Boolean) {
        for (tradeRequest in civInfo.tradeRequests.toList()) {
            ...
            // Territorial Warfare: civs with no cities auto-accept peace treaties (bypass validation)
            if (civInfo.cities.isEmpty() && tradeRequest.trade.isPeaceTreaty()) {
                civInfo.tradeRequests.remove(tradeRequest)
                val tradeLogic = TradeLogic(civInfo, otherCiv)
                tradeLogic.currentTrade.set(tradeRequest.trade)
                tradeLogic.acceptTrade()
                otherCiv.addNotification("[${civInfo.civName}] has accepted your peace offer", NotificationCategory.Trade, NotificationIcon.Trade, civInfo.civName)
                continue
            }
