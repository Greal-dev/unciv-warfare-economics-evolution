...
class CityConstructions : IsPartOfGameInfoSerialization {
    ...
    fun getProductionMarkup(ruleset: Ruleset): FormattedLine {
        ...
        if (ProductionTransferConstruction.isTransferConstruction(currentConstructionSnapshot)) {
            val construction = getConstruction(currentConstructionSnapshot) as ProductionTransferConstruction
            return FormattedLine("{${construction.getDisplayName()}}")
        }
        val category = when {
    ...
    internal fun getConstruction(constructionName: String): IConstruction {
        ...
        // Handle production transfer constructions
        val targetCityId = ProductionTransferConstruction.parseCityId(constructionName)
        if (targetCityId != null) {
            val targetCity = city.civ.cities.firstOrNull { it.id == targetCityId }
            val targetName = targetCity?.name ?: "Unknown"
    ...
    fun endTurn(cityStats: Stats) {
        ...
        if (currentConstruction is ProductionTransferConstruction) {
            val targetCity = city.civ.cities.firstOrNull { it.id == currentConstruction.targetCityId }
            if (targetCity != null) {
                val efficiency = ProductionTransferConstruction.getEfficiency(city.civ)
                val transferred = ((cityStats.production.roundToInt() + productionOverflow) * efficiency).roundToInt()
                targetCity.cityConstructions.productionOverflow += transferred
